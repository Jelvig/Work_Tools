...
This script also works for Condensations, just ignore the Probe errors."""
class CoolerRaw():
    def file_path(self):
    ...
    def RP_split(self):
        ...
        if 2 in self.special:
            ...
            #  Creating RP nonhuman split
            self.RPnonHs = RP.drop(RP[RP['Description 2'] == 'Hs'].index)
            self.RPnonHs = self.RPnonHs.sort_values(by='Bin Code')
        else:
            print("No RP's to split")
            
    def GP_split(self):
        #  Creating GP human split
        if 3 in self.special:
            ...
            self.GPnonHs = GP.drop(GP[GP['Description 2'] == 'Hs'].index)
    ...
    def df_writer(self):
        ...
        with pd.ExcelWriter(self.file, engine='xlsxwriter') as writer:
            ...
            if 2 in self.special:
                ...
                self.RPnonHs.to_excel(writer, sheet_name='RPnonHs', index=False)
            if 3 in self.special:
                self.GPHs.to_excel(writer, sheet_name='GPHs', index=False)
                self.GPnonHs.to_excel(writer, sheet_name='GPnonHs', index=False)
            if 4 in self.special:
